...
import { OpenLinkMemberStruct } from "../talk/struct_old/open/open-link-struct";
import { OpenProfileType } from "../talk/open_old/open-link-type";
...
export class PacketUpdateLinkProfileRes extends LocoBsonResponsePacket {
    ...
    constructor(
        status: number,
        public UpdatedProfile?: OpenLinkMemberStruct
    ) {
        super(status);
    }
    ...
    readBodyJson(rawData: any) {
        if (rawData['olu']) {
            this.UpdatedProfile = Serializer.deserialize<OpenLinkMemberStruct>(rawData['olu'], OpenLinkMemberStruct.MAPPER);
        }
    }
